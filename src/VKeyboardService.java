...
// 音声キーボードサービスプログラム for android  ver 0.2.4
// 変更点：
...
public void recognize( double[] voice) {
  ...
  if( max > thresh_recognize ){
    ...
    // 音声認識が有効な場合
      ...
      // タイプするキーコードをセットする(複数のキーコードに対応、ただしキーコードがマイナスの場合は無効)
      int key = vt.code;
      if(key < 0) return;
      do{
        type(key & 0xff);
        try{ Thread.sleep(100); } catch(Exception e){}  // 0.1秒待つ
        key >>= 8;
      } while(key > 0 );
    }
  }
